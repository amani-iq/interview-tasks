# Fullstack / NestJS + Next.js — Senior take-home
...
| --- | --- |
| Track | Fullstack |
| Stack | NestJS API (PostgreSQL) + Next.js (App Router) UI |
...
Vendors send signed webhooks to a NestJS API. The API stores events and work items. Ops users in a **workspace** inspect events, retry dead items, and must never see another workspace’s data.
...
- **NestJS** + PostgreSQL for ingest and reads.
