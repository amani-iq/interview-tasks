# Fullstack / PHP (Laravel) + Next.js — Senior take-home
...
| --- | --- |
| Track | Fullstack |
| Stack | Laravel API (MySQL/PostgreSQL) + Next.js (App Router) UI |
...
Vendors send signed webhooks to a Laravel API. The API stores events and work items. Ops users in a **workspace** inspect events, retry dead items, and must never see another workspace’s data.
...
- **Laravel** + MySQL/PostgreSQL for ingest and reads.
...
1. **Ingest API** (`POST /api/webhooks/{vendor}`)
   ...
   - Persist raw event + `work_item` (`pending` → `done` | `failed` → `dead`)
...
4. **Logs:** `tenant_id`, `vendor`, `event_id`. No raw secrets.
...
- Prefer a repo with one `README` and clear scripts (queue worker included).
