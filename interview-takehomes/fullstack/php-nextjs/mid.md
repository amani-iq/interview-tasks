# Fullstack / PHP (Laravel) + Next.js — Mid take-home
...
Employees create leave requests. A manager approves or rejects. Two people on the same team must not both be **approved** off on the same day if the team is `coverage_required: true`. Each employee has a yearly balance (weekdays only).

The **Laravel API owns the rules**. The UI must show API errors in human language.
...
- Next.js for the UI.
- **Laravel** for the API. Rules live in Laravel services/actions, not in Route Handlers or the React form.
...
2. API:
   - Create request (`start_date`, `end_date`, `type`: `vacation` | `sick`)
...
- Two processes or a repo with clear scripts
- Migrations + seeders
...
- A shared error-code list the UI maps to copy
- Laravel tests for the coverage rule plus a short note on CORS/proxy
