# Fullstack / ASP.NET Core + Next.js — Mid take-home
...
The **ASP.NET Core API owns the rules**. The UI must show API errors in human language.
...
- Next.js for the UI.
- **ASP.NET Core** for the API. Rules live in .NET services, not in Route Handlers or the React form.
...
- .NET tests for the coverage rule plus a short note on CORS/proxy
