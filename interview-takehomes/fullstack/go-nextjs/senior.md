# Fullstack / Go + Next.js — Senior take-home
...
| --- | --- |
| Track | Fullstack |
| Stack | Go API (PostgreSQL) + Next.js (App Router). Next-only API is a downgrade. |
...
Vendors send signed webhooks. The API stores events and work items. Ops users in a **workspace** inspect events, retry dead items, and must never see another workspace’s data.
...
- **Go** + PostgreSQL for ingest and reads (`net/http` or a small router).
...
1. **Ingest API** (`POST /webhooks/:vendor`)
   - HMAC signature (document algorithm and signed string)
...
- Prefer a monorepo or two folders with one `README` and compose/`npm` scripts.
