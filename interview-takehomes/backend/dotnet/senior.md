# Backend / .NET (ASP.NET Core) — Senior take-home
...
| --- | --- |
| Track | Backend |
| Stack | ASP.NET Core (.NET 8), C#, PostgreSQL |
...
6–8 hours. Prefer one production-shaped slice over a tour of .NET features.

### Rules of the exercise

- Use ASP.NET Core + PostgreSQL.
- You may fake downstream workers (in-process `BackgroundService`, queue table, or a channel). Say why.
...
- If you use a background worker or queue, explain what happens when the process dies mid-handler.
- Keep the code boring and readable. Clever middleware is not a substitute for a clear ingest use-case.
